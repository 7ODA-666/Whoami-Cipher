...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
    <a href="{{ route('caesar.encryption') }}" 
       class="block bg-gray-800 border border-gray-700 rounded-lg p-6 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/20 hover:-translate-y-1 transition-all">
        <div class="flex items-center gap-3 mb-2">
            <i class="fas fa-key text-2xl text-blue-400"></i>
            <h3 class="text-xl font-bold text-blue-400">Caesar Cipher</h3>
        </div>
        <p class="text-gray-400 text-sm">A simple substitution cipher where each letter is shifted by a fixed number of positions in the alphabet.</p>
    </a>
    
    <a href="{{ route('monoalphabetic.encryption') }}" 
       class="block bg-gray-800 border border-gray-700 rounded-lg p-6 hover:border-purple-500 hover:shadow-lg hover:shadow-purple-500/20 hover:-translate-y-1 transition-all">
        <div class="flex items-center gap-3 mb-2">
        ...
        <p class="text-gray-400 text-sm">A substitution cipher using a fixed permutation of the alphabet for the entire message.</p>
    </a>
    
    <a href="{{ route('polyalphabetic.encryption') }}" 
       class="block bg-gray-800 border border-gray-700 rounded-lg p-6 hover:border-orange-500 hover:shadow-lg hover:shadow-orange-500/20 hover:-translate-y-1 transition-all">
        <div class="flex items-center gap-3 mb-2">
        ...
        <p class="text-gray-400 text-sm">A cipher that uses multiple substitution alphabets based on a keyword, making it more secure than monoalphabetic ciphers.</p>
    </a>
    ...
    <a href="{{ route('one-time-pad.encryption') }}" 
       class="block bg-gray-800 border border-gray-700 rounded-lg p-6 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/20 hover:-translate-y-1 transition-all">
        ...
        <p class="text-gray-400 text-sm">The only theoretically unbreakable cipher, using a random key of the same length as the plaintext.</p>
    </a>
    ...
    <a href="{{ route('playfair.encryption') }}" 
       class="block bg-gray-800 border border-gray-700 rounded-lg p-6 hover:border-purple-500 hover:shadow-lg hover:shadow-purple-500/20 hover:-translate-y-1 transition-all">
        ...
        <p class="text-gray-400 text-sm">A digraph substitution cipher that encrypts pairs of letters using a 5×5 matrix constructed from a keyword.</p>
    </a>
    ...
    <a href="{{ route('hill.encryption') }}" 
       class="block bg-gray-800 border border-gray-700 rounded-lg p-6 hover:border-orange-500 hover:shadow-lg hover:shadow-orange-500/20 hover:-translate-y-1 transition-all">
        ...
        <p class="text-gray-400 text-sm">A polygraphic substitution cipher based on linear algebra, encrypting blocks of letters using matrix multiplication.</p>
    </a>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
    <a href="{{ route('rail-fence.encryption') }}" 
       class="block bg-gray-800 border border-gray-700 rounded-lg p-6 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/20 hover:-translate-y-1 transition-all">
        ...
        <p class="text-gray-400 text-sm">A transposition cipher that writes the plaintext in a zigzag pattern along multiple "rails" and reads it row by row.</p>
    </a>
    ...
    <a href="{{ route('row-column-transposition.encryption') }}" 
       class="block bg-gray-800 border border-gray-700 rounded-lg p-6 hover:border-purple-500 hover:shadow-lg hover:shadow-purple-500/20 hover:-translate-y-1 transition-all">
        ...
        <p class="text-gray-400 text-sm">A transposition cipher that writes the plaintext row by row and reads it column by column based on a keyword's alphabetical order.</p>
    </a>
</div>
@endsection
