...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 mb-6 lg:mb-8">
    <div class="bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl p-4 lg:p-6 shadow-xl">
        ...
        <label for="rails-input-decrypt" class="block mt-4 mb-2 text-light-text dark:text-dark-text font-semibold">Number of Rails (2-10)</label>
        <input
            type="number"
            id="rails-input-decrypt"
            min="2"
            max="10"
            value="3"
        ...
        />
        <p class="text-xs text-light-text-secondary dark:text-dark-text-secondary mt-2">Same number of rails used for encryption.</p>
    </div>
...
function executeDecrypt() {
    ...
    const keyField = document.getElementById('rails-input-decrypt');
    const outputField = document.getElementById('plaintext-output');
    const vizContent = document.querySelector('.visualization-content');

    executeCryptoAjax('rail-fence', 'decrypt', inputField, keyField, outputField, vizContent);
}
</script>
@endpush
@endsection
