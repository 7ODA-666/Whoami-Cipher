...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 mb-6 lg:mb-8">
    <div class="bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl p-4 lg:p-6 shadow-xl">
        ...
        <textarea
            ...
            placeholder="Enter plaintext (letters only, no J)"
            rows="6"
        ></textarea>

        <label for="keyword-input" class="block mt-4 mb-2 text-light-text dark:text-dark-text font-semibold">Keyword</label>
...
function executeEncrypt() {
    ...
    const keyField = document.getElementById('keyword-input');
    const outputField = document.getElementById('output-text');
    const vizContent = document.querySelector('.visualization-content');

    executeCryptoAjax('playfair', 'encrypt', inputField, keyField, outputField, vizContent);
}
</script>
@endpush
@endsection
